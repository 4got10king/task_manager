...
class TasksService:
    ...
    async def get_task_by_id(
        cls, task_id: int, uow: IUnitOfWork = UnitOfWork()
    ) -> TasksResponse:
        async with uow:
    ...
    async def create_task(
        cls, task_data: TasksCreate, uow: IUnitOfWork = UnitOfWork()
    ) -> TasksResponse:
        async with uow:
    ...
    async def update_task(
        cls, task_id: int, task_data: TasksUpdate, uow: IUnitOfWork = UnitOfWork()
    ) -> TasksResponse:
        async with uow:
            task = await uow.tasks.get_by_id(task_id)
            if not task:
